...
Router.route('/managepost',{
    ...
    waitOn:function(){
        ...
        return Meteor.subscribe("PostPageList",{skip: 0},page);
    },
    data:function(){
        if(this.ready){
...
Router.route('/editpost/:id',{
    ...
    data:function(){
        if(this.ready){
            var result = post.findOne({_id:this.params.id});
            Session.set('date_update',result.date_created * 1000);
            return result;
        }
    }
})
...
Router.route('/partner',{
    name:'partner',
    waitOn:function(){
        Meteor.subscribe('pages','partner');
    },
    data:function(){
        if(this.ready){
            return post.findOne({'location':'partner'});
        }
    }
})
